...
public class app_db 
{
	...
	
    {
		...
		do
		{
			...
			{
				request = new request_response(config.getProperty("request_location")+input.read_data("testdata")+"_request"+".json");
				...
				response = new request_response(config.getProperty("response_location")+input.read_data("testdata")+"_response"+".json");  // response location
